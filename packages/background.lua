...
requires at least one parameter, \autodoc:param{color}, and sets the backgound of the
current and all following pages to that color. The color specification in the same as
specified in the \autodoc:package{color} package.

If setting only the current page background different from the default is desired, an
extra parameter \autodoc:param{allpages} can be set to false.

\background[color=#e9d8ba,allpages=false]

So, for example, \autodoc:code{\\background[color=#e9d8ba,allpages=false]} will set a
