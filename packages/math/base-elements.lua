...
function elements.sqrt:shape ()
   ...
   local ratio = (self.radicand.height:tonumber() + self.radicand.depth:tonumber()) / (radicalGlyph.height + radicalGlyph.depth)
   self.symbolHeight = SILE.types.length(radicalGlyph.height)
   self.symbolDepth = SILE.types.length(radicalGlyph.depth) + (ratio > 1 and math.log(ratio) or 0) * self.radicalVerticalGap * scaleDown
   self.symbolWidth = SILE.types.length(radicalGlyph.width) + (ratio > 1 and math.log(ratio) or 0) * self.radicalVerticalGap * scaleDown
...
function elements.sqrt:output (x, y, line)
   ...
   SILE.outputter:drawRule(
      s0 + self.symbolWidth + scaleWidth(x, line),
