...
\autodoc:code{.bib} files and output citations and full text references.
(It doesn’t currently produce full bibliography listings.)

To load a BibTeX file, issue the command
\autodoc:code{\\loadbibliography[file=whatever.bib]}

To produce an inline citation, call \autodoc:code{\\cite\{key\}}, which
will typeset something like “Jones 1982”. If you want to cite a
particular page number, use \autodoc:code{\\cite[page=22]\{key\}}.

To produce a full reference, use \autodoc:code{\\reference\{key\}}.
...
Check out \autodoc:code{packages/bibstyles/chicago.lua} and adapt as necessary.
\end{document}]]
}
